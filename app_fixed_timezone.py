...
def format_ist_time(dt):
    ...
    try:
        # If it's naive, assume it's already in IST
...
def get_time_ago(dt):
    ...
    try:
        ...
        # If dt is naive, assume it's in IST
        if dt.tzinfo is None:
...
def predict_stock(user):
    try:
        ...
        current_ist_naive = get_ist_naive()
        
        print(f"🔮 Making prediction for {ticker} using {model_type} model")
        print(f"🕐 Current IST: {format_ist_time(current_ist)}")
        
        # Get prediction from ML model
        result = predictor.predict_stock(ticker, model_type, days_ahead)
        
        if result['success']:
            # Save prediction to database with IST time
            try:
                ...
                prediction = PredictionHistory(
                    ...
                    created_at=current_ist_naive  # Store IST as naive datetime
                )
                ...
                db.session.commit()
                print(f"✅ Prediction saved at IST: {format_ist_time(current_ist_naive)}")
...
def prediction_history(user):
    try:
        ...
        current_ist = get_current_ist()
        print(f"📊 Loading history at IST: {format_ist_time(current_ist)}")
        print(f"📊 Found {len(predictions_raw)} predictions for user {user.username}")
        ...
        for pred in predictions_raw:
            try:
                ...
                processed_pred = {
                    ...
                    'formatted_time': format_ist_time(pred.created_at),
                    'time_ago_str': get_time_ago(pred.created_at)
                }
                
                processed_predictions.append(processed_pred)
                print(f"✅ Processed prediction {pred.id}: {pred.ticker} at {processed_pred['formatted_time']}")
        ...
        return render_template('history_ist.html', predictions=processed_predictions)
        ...
        return render_template('history_ist.html', predictions=[])
...
if __name__ == '__main__':
    ...
    print(f"🕐 Current IST Time: {format_ist_time(current_ist)}")
    print(f"🌏 Timezone: Asia/Kolkata (UTC+05:30)")
    print("\n💡 Supported Markets:")
